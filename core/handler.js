...
// ======= Основная логическая функция (общая)） =======
...
function errorLogs(err, extra = {}) {
    ...
    try {
        ...
        // Judgment Vercel / Node.js Environment
        else if (typeof process !== 'undefined' && process.release?.name === 'node') {
            prefix = '[VC-ERR]';
        }
        else {
...
async function parseIpUrl(ip_url) {
    ...
    try {
        ...
        errorLogs('Get IP_URL File content failed to load：', err);
        return { ipUrlCsvResult: [], ipUrlTxtResult: [] };
    }
}
...
function getHtmlRes(rawHost, proxyIP, socks5Enable, parsedSocks5, host, v2, clash) {
    ...
    let remark = `Ваш узел подписки определяется путем настройки переменных ${subRemark} Поставлять, В настоящее время используется обратный прокси-сервер${proxyIPRemark}`;
    if (!proxyIP && !socks5Enable) {
        remark = `Ваш узел подписки определяется путем настройки переменных ${subRemark} Поставлять, В настоящее время обратный прокси-сервер не установлен, Мы рекомендуем вам установитьPROXYIPПеременная илиSOCKS5Переменная илиДиапазон подключения подпискиproxyIP`;
    }
...
function getConfigLink(uuid, host, address, port, remarks, proxyip, protType, nat64) {
    ...
    if (host.includes('.workers.dev') || host.includes('pages.dev')) {
        path = `/${host}${path}`;
        remarks += ' Пожалуйста, войдите и подпишитесь, используя свой личный домен!';
    }
...
function getConfigHtml(host, remark, v2, clash) {
    ...
    const htmlHead = `
        ...
        <meta name='description' charset='UTF-8' content='This is a project to generate free vmess nodes ' />
        <style>
            body {
    ...
    const header = `
        <div class="links">
            <div class="link-row">
                <a href="https://${host}/${id}/setting" rel="noopener">⚙️ Пользовательские настройки</a>
            </div>
        </div>
  `;
    ...
    const output = cleanLines(`
        ################################################################
        ---------------------------------------------------------------
        "Общий" адрес подписки: <button onclick='copyToClipboard("${httpAddr}?sub")'><i class="fa fa-clipboard"></i> Нажмите, чтобы скопировать</button>
        ${httpAddr}?sub

        "Base64" адрес подписки: <button onclick='copyToClipboard("${httpAddr}?base64")'><i class="fa fa-clipboard"></i> Нажмите, чтобы скопировать</button>
        ${httpAddr}?base64

        "Clash" адрес подписки: <button onclick='copyToClipboard("${httpAddr}?clash")'><i class="fa fa-clipboard"></i> Нажмите, чтобы скопировать</button>
        ${httpAddr}?clash

        "Singbox" адрес подписки: <button onclick='copyToClipboard("${httpAddr}?singbox")'><i class="fa fa-clipboard"></i> Нажмите, чтобы скопировать</button>
        ${httpAddr}?singbox
        ---------------------------------------------------------------
        ################################################################
        v2
...
async function getSettingHtml(host) {
    const title = decodeBase64Utf8(fileName);
    const fullTitle = title + '-Пользовательские настройки';
    ...
    <div class="container">
        ...
        <div class="navbar-left">
            <button class="back-btn" onclick="goHome()">🏠 Вернуться на главную страницу</button>
        </div>
	    </div>

        <form id="configForm">
        <h2 class="form-title"><span class="icon">⚙️</span> Пользовательские настройки</h2>
        ...
        <label>UUID</label>
        <input type="text" id="UUID" name="HOUUIDST" placeholder="Обязательно：UUID (Например：d0298536-d670-4045-bbb1-ddd5ea68683e)" />

        <label>HOST</label>
        <input type="text" id="HOST" name="HOST" placeholder="Обязательно：Узел домена Cloudflare (Например：vless.test.com)" />

        <label>IP_URL</label>
        <input type="text" id="IP_URL" name="IP_URL" placeholder="Необязательно：Предпочтительный IP-адрес (Например：https://raw.github.../ip.txt)" />

        <label>PROXYIP</label>
        <input type="text" id="PROXYIP" name="PROXYIP" placeholder="Необязательно：Обратный прокси-IP, домен или адрес (Например：proxyip.test.test.com)" />

        <label>SOCKS5</label>
        <input type="text" id="SOCKS5" name="SOCKS5" placeholder="Необязательно：SOCKS5 (Например：socks5://user:pass@ip:port)" />

        <label>SUB_CONFIG</label>
        <input type="text" id="SUB_CONFIG" name="SUB_CONFIG" placeholder="Необязательно：Файл конфигурации преобразования подписки (Например：https://raw.github...)" />
        <label>SUB_CONVERTER</label>
        <input type="text" id="SUB_CONVERTER" name="SUB_CONVERTER" placeholder="Необязательно：Адрес API бэкэнда конвертации подписки (Например：url.v1.mk)" />

        <label>NAT64_PREFIX</label>
        <input type="text" id="NAT64_PREFIX" name="NAT64_PREFIX" placeholder="Необязательно：NAT64 Префикс (Например：2602:fc59:b0:64::)" />
        <label>NAT64</label>
        ...
        <input type="text" id="HOST_REMARK" name="HOST_REMARK" placeholder="Необязательно：Названия узлов" />
        
        <button type="button" class="save-btn" onclick="saveSettings()">💾 Сгенерировать ссылку</button>
        </form>
    </div>
    ...
    <script>
        ...
        let hasError = false;
        if (!uuid) {
            showError('UUID', 'Пожалуйста, заполните UUID');
            hasError = true;
        }
        if (!hostInput) {
            showError('HOST', 'Пожалуйста, заполните HOST');
            hasError = true;
        }
        if (hasError) return; 
...
async function login(req, env, res = null) {
    ...
    const renderLoginPage = (heading, status = 200) => {
        const html = renderPage({
            base64Title: pName,
            suffix: '-Авторизоваться',
            heading,
            bodyContent: `
                <form method="POST">
                    <input type="password" name="password" placeholder="Введите пароль доступа" required />
    ...
    if (method === "GET") return renderLoginPage('🔐 Пожалуйста, введите ваш пароль для авторизации');
    ...
    if (method === "POST") {
        ...
        log(`[LOGIN] → POST Введите пароль: "${inputPassword}"`);

        if (inputPassword === id) {
            log(`[LOGIN] → Пароль верный`);
            if (!uuid || !host) {
                return renderLoginPage(`❌ UUIDилиHOSTПеременная не установлена`, 400);
            }
            log(`[LOGIN] → Перейти к id=${id}`);
            return redirectToId(id, req, res);
        } else {
            log(`[LOGIN] → Неверный пароль`);
            return renderLoginPage('❌ Неверный пароль，Пожалуйста, попробуйте еще раз', 403);
        }
    }
...
async function redirectToId(id, req, res = null) {
    ...
    // Node / Vercel
    if (res) {
        log(`[redirectToId] → Node/Vercel Перенаправить на /${id}`);
        res.writeHead(302, { Location: `/${id}` });
        res.end();
        return { status: 302, text: async () => '' }; // Возврат объекта，предотвращать mainHandler crash
    }

    // Edge / CF Worker
    const fullUrl = new URL(req.url, `https://${req.headers.get('host') || 'localhost'}`);
    log(`[redirectToId] → CF Worker Перенаправить на ${fullUrl.origin}/${id}`);
